...
func TestRun_UnsupportedStorageType(t *testing.T) {
	err := run(serverConfig{StorageType: "redis", APIKey: "test-key"})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "unsupported storage type: redis")
}

func TestRun_RejectsEmptyAPIKeyWithoutUnsafeFlag(t *testing.T) {
	err := run(serverConfig{StorageType: "sqlite", DBPath: filepath.Join(t.TempDir(), "sync.db")})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "--api-key is required unless --unsafe-no-auth is set")
}

func TestRun_AllowsExplicitUnsafeNoAuth(t *testing.T) {
	dir := t.TempDir()
	...
	require.Error(t, err)
	assert.NotContains(t, err.Error(), "--api-key is required unless --unsafe-no-auth is set")
}

func TestRun_PostgresWithoutURL(t *testing.T) {
	err := run(serverConfig{StorageType: "postgres", PostgresURL: "", APIKey: "test-key"})
	require.Error(t, err)
